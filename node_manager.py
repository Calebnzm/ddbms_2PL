...
class NodeManager:
    ...
    def _initialize_nodes(self):
        for node_name, db_file in self.node_files.items():
            ...
            conn.close()
            if new_db:
                print(f"Node '{node_name}' created at {db_file}.")
    ...
    def create_account(self, city: str, initial_balance: int = 1000, account_id: int = None) -> int:
        ...
        self.account_index[account_id] = node_name
        print(f"Account '{account_id}' added to node '{node_name}' with balance {initial_balance}")
        return account_id

    def delete_account(self, account_id: int) -> None:
        node_name = self.get_node_for_account(account_id)
        if node_name is None:
            print(f"No node found for account '{account_id}'")
            return
        conn = sqlite3.connect(self.node_files[node_name])
        cursor = conn.cursor()
        ...
        print(f"Account '{account_id}' deleted from node '{node_name}'")
    ...
    def write_balance(self, account_id: int, balance: int) -> None:
        ...
        conn.commit()
        conn.close()
        print(f"Account '{account_id}' balance updated to {balance} on node '{node_name}'")
